...
#include "LIS_misc.h"
!BOP
! !MODULE: genEnsFcst_VariablesMod
! \label{genEnsFcst_VariablesMod}
! 
! !DESCRIPTION: 
...
!------------------------------------------------------------------------------
  public :: genEnsFcst_Variables_init   ! initialize forcing variables
  public :: genEnsFcst_Variables_read   ! Read NetCDF forcing variables
!  public :: genEnsFcst_Variables_reset  ! deallocate/reinit any variable arrays/inputs
!  public :: genEnsFcst_Variables_final  ! deallocate any variable arrays/inputs
!EOP

!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  public :: ensfcstvars_struc
  public :: forcopts
!EOP
  ...
  type(forcvars_type_dec), allocatable :: ensfcstvars_struc(:)
...
!BOP
! !ROUTINE: genEnsFcst_Variables_init 
!  \label{genEnsFcst_Variables_init}
! 
! !INTERFACE: 
!
 subroutine genEnsFcst_Variables_init( findex, ftn, &
...
! _______________________________________________
   ...
   allocate( ensfcstvars_struc(LIS_rc%nnest) )
...
!- Allocate and initialize all read-in forcing variable fields:
      ...
      ! AIRTEMP:
        if( LIS_FORC_Tair%selectOpt == 1 ) then  ! Air temp. field selected
          if( varName == "T2M" ) then  ! Air temp. field present 
            write(LIS_logunit,*) &
             "[INFO] GenEnsFcst variable being read in: ",trim(varName)
            num_vars = num_vars + 1
            do n=1, LIS_rc%nnest
               allocate( ensfcstvars_struc(n)%airtmp(&
                         LIS_rc%lnc(n),LIS_rc%lnr(n) ) )
               ensfcstvars_struc(n)%airtmp = LIS_rc%udef
            end do
      ...
      ! SPECHUM (Q2):
        if( LIS_FORC_Qair%selectOpt == 1 ) then  ! Spec. humidity field selected
          if( varName == "QV2M" ) then   ! field present 
            ...
            num_vars = num_vars + 1
            do n=1, LIS_rc%nnest
               allocate( ensfcstvars_struc(n)%spechum(&
                         LIS_rc%lnc(n),LIS_rc%lnr(n) ) )
               ensfcstvars_struc(n)%spechum = LIS_rc%udef
            end do
      ...
      ! Downward shortwave radiation (SWGDN):
        if( LIS_FORC_SWdown%selectOpt == 1 ) then   ! field selected
          if( varName == "SWGDN" ) then   ! field present 
            ...
            num_vars = num_vars + 1
            do n=1, LIS_rc%nnest
               allocate( ensfcstvars_struc(n)%swdown(&
                         LIS_rc%lnc(n),LIS_rc%lnr(n) ) )
               ensfcstvars_struc(n)%swdown = LIS_rc%udef
            end do
      ...
      ! Downward longwave radiation (LWGAB):
        if( LIS_FORC_LWdown%selectOpt == 1 ) then   ! field selected
          if( varName == "LWGAB" ) then   ! field present 
            ...
            num_vars = num_vars + 1
            do n=1, LIS_rc%nnest
               allocate( ensfcstvars_struc(n)%lwdown(&
                         LIS_rc%lnc(n),LIS_rc%lnr(n) ) )
               ensfcstvars_struc(n)%lwdown = LIS_rc%udef
            end do
      ...
      ! Wind E-dir (U10M):
        if( LIS_FORC_Wind_E%selectOpt == 1 ) then   ! field selected
          if( varName == "U10M" ) then   ! field present 
            ...
            num_vars = num_vars + 1
            do n=1, LIS_rc%nnest
               allocate( ensfcstvars_struc(n)%uwind(&
                         LIS_rc%lnc(n),LIS_rc%lnr(n) ) )
               ensfcstvars_struc(n)%uwind = LIS_rc%udef
            end do
      ...
      ! Wind - North direction (V10M):
        if( LIS_FORC_Wind_N%selectOpt == 1 ) then   ! field selected
          if( varName == "V10M" ) then   ! field present 
            ...
            num_vars = num_vars + 1
            do n=1, LIS_rc%nnest
               allocate( ensfcstvars_struc(n)%vwind(&
                         LIS_rc%lnc(n),LIS_rc%lnr(n) ) )
               ensfcstvars_struc(n)%vwind = LIS_rc%udef
            end do
      ...
      ! Surface Pressure (PS):
        if( LIS_FORC_Psurf%selectOpt == 1 ) then   ! field selected
          if( varName == "PS" ) then   ! field present 
            ...
            num_vars = num_vars + 1
            do n=1, LIS_rc%nnest
               allocate( ensfcstvars_struc(n)%psurf(&
                         LIS_rc%lnc(n),LIS_rc%lnr(n) ) )
               ensfcstvars_struc(n)%psurf = LIS_rc%udef
            end do
      ...
      ! RAINFALL:
        if( LIS_FORC_Rainf%selectOpt == 1 ) then  ! Rainfall field selected
          if( varName == "PRECTOT" ) then  ! Rainfall field present 
            ...
            num_vars = num_vars + 1
            do n=1, LIS_rc%nnest
               allocate( ensfcstvars_struc(n)%rainf(&
                         LIS_rc%lnc(n),LIS_rc%lnr(n) ) )
               ensfcstvars_struc(n)%rainf = LIS_rc%udef
            end do
      ...
      ! Convective Rainfall (PRECCON):
        if( LIS_FORC_CRainf%selectOpt == 1 ) then   ! field selected
          if( varName == "PRECCON" ) then   ! field present 
            ...
            num_vars = num_vars + 1
            do n=1, LIS_rc%nnest
               allocate( ensfcstvars_struc(n)%cpcp(&
                         LIS_rc%lnc(n),LIS_rc%lnr(n) ) )
               ensfcstvars_struc(n)%cpcp = LIS_rc%udef
            end do
 ...
 end subroutine genEnsFcst_Variables_init

!BOP
! !ROUTINE: genEnsFcst_Variables_read
!  \label{genEnsFcst_Variables_read}
! 
! !INTERFACE: 
!
 subroutine genEnsFcst_Variables_read(findex, filename, inc, inr, tindex )
...
!- Open and check GenEnsFcst netcdf file:
    ios = nf90_open(path=filename,&
          mode=NF90_NOWRITE,ncId=nid)
    call LIS_verify(ios,'Error in nf90_open in genEnsFcst_Variables_read')

!- Read in GenEnsFcst netcdf file:

  ! Read-in and spatially reproject Air temperature:
    if( forcopts%read_airtmp ) then
      ios = nf90_inq_varid( nid, "T2M", varid )
      call LIS_verify(ios,'Error in nf90_inq_varid(T2M) in genEnsFcst_Variables_read')
...
!              start=(/start_inc, start_inr, tindex/),&
              start=(/1, 1, tindex/),&
              count=(/inc, inr, 1/) )
      call LIS_verify(ios,'Error in nf90_get_var(T2M) in genEnsFcst_Variables_read')
      do n = 1, LIS_rc%nnest
         call genEnsFcst_interp_data( n, findex, inc, inr, tindex,&
                 input_var, .false., ensfcstvars_struc(n)%airtmp )
      end do
    endif
  ...
  ! Read-in and spatially reproject Spec Humidity:
    if( forcopts%read_spechum ) then
      ...
      call LIS_verify(ios,'Error in nf90_inq_varid(QV2M) in genEnsFcst_Variables_read')
...
!              start=(/start_inc, start_inr, tindex/),&
              start=(/1, 1, tindex/),&
              count=(/inc, inr, 1/) )
      call LIS_verify(ios,'Error in nf90_get_var(QV2M) in genEnsFcst_Variables_read')
      do n = 1, LIS_rc%nnest
         call genEnsFcst_interp_data( n, findex, inc, inr, tindex,&
                 input_var, .false., ensfcstvars_struc(n)%spechum )
      end do
    endif
  ...
  ! Read-in and spatially reproject SW Down:
    if( forcopts%read_swdown ) then
      ...
      call LIS_verify(ios,'Error in nf90_inq_varid(SWGDN) in genEnsFcst_Variables_read')
...
!              start=(/start_inc, start_inr, tindex/),&
              start=(/1, 1, tindex/),&
              count=(/inc, inr, 1/) )
      call LIS_verify(ios,'Error in nf90_get_var(SWGDN) in genEnsFcst_Variables_read')
      do n = 1, LIS_rc%nnest
         call genEnsFcst_interp_data( n, findex, inc, inr, tindex,&
                 input_var, .false., ensfcstvars_struc(n)%swdown )
      end do
    endif
  ...
  ! Read-in and spatially reproject LWdown:
    if( forcopts%read_lwdown ) then
      ...
      call LIS_verify(ios,'Error in nf90_inq_varid(LWGAB) in genEnsFcst_Variables_read')
...
!              start=(/start_inc, start_inr, tindex/),&
              start=(/1, 1, tindex/),&
              count=(/inc, inr, 1/) )
      call LIS_verify(ios,'Error in nf90_get_var(LWGAB) in genEnsFcst_Variables_read')
      do n = 1, LIS_rc%nnest
         call genEnsFcst_interp_data( n, findex, inc, inr, tindex,&
                 input_var, .false., ensfcstvars_struc(n)%lwdown )
      end do
    endif
  ...
  ! Read-in and spatially reproject U-wind:
    if( forcopts%read_uwind ) then
      ...
      call LIS_verify(ios,'Error in nf90_inq_varid(U10M) in genEnsFcst_Variables_read')
...
!              start=(/start_inc, start_inr, tindex/),&
              start=(/1, 1, tindex/),&
              count=(/inc, inr, 1/) )
      call LIS_verify(ios,'Error in nf90_get_var(U10M) in genEnsFcst_Variables_read')
      do n = 1, LIS_rc%nnest
         call genEnsFcst_interp_data( n, findex, inc, inr, tindex,&
                 input_var, .false., ensfcstvars_struc(n)%uwind )
      end do
    endif
  ...
  ! Read-in and spatially reproject V-wind:
    if( forcopts%read_vwind ) then
      ...
      call LIS_verify(ios,'Error in nf90_inq_varid(V10M) in genEnsFcst_Variables_read')
...
!              start=(/start_inc, start_inr, tindex/),&
              start=(/1, 1, tindex/),&
              count=(/inc, inr, 1/) )
      call LIS_verify(ios,'Error in nf90_get_var(V10M) in genEnsFcst_Variables_read')
      do n = 1, LIS_rc%nnest
         call genEnsFcst_interp_data( n, findex, inc, inr, tindex,&
                 input_var, .false., ensfcstvars_struc(n)%vwind )
      end do
    endif
  ...
  ! Read-in and spatially reproject PS:
    if( forcopts%read_psurf ) then
      ...
      call LIS_verify(ios,'Error in nf90_inq_varid(PS) in genEnsFcst_Variables_read')
...
!              start=(/start_inc, start_inr, tindex/),&
              start=(/1, 1, tindex/),&
              count=(/inc, inr, 1/) )
      call LIS_verify(ios,'Error in nf90_get_var(PS) in genEnsFcst_Variables_read')
      do n = 1, LIS_rc%nnest
         call genEnsFcst_interp_data( n, findex, inc, inr, tindex,&
                 input_var, .false., ensfcstvars_struc(n)%psurf )
      end do
    endif
  ...
  ! Read-in and spatially reproject Rainfall:
    if( forcopts%read_rainf ) then
      ...
      call LIS_verify(ios,'Error in nf90_inq_varid(PRECTOT) in genEnsFcst_Variables_read')
...
!              start=(/start_inc, start_inr, tindex/),&
              start=(/1, 1, tindex/),&
              count=(/inc, inr, 1/) )
      call LIS_verify(ios,'Error in nf90_get_var(PRECTOT) in genEnsFcst_Variables_read')
      do n = 1, LIS_rc%nnest
         call genEnsFcst_interp_data( n, findex, inc, inr, tindex,&
                 input_var, .true., ensfcstvars_struc(n)%rainf )
      end do   
    endif
  ...
  ! Read-in and spatially reproject Conv. Rainfall:
    if( forcopts%read_cpcp ) then
      ...
      call LIS_verify(ios,'Error in nf90_inq_varid(PRECCON) in genEnsFcst_Variables_read')
...
!              start=(/start_inc, start_inr, tindex/),&
              start=(/1, 1, tindex/),&
              count=(/inc, inr, 1/) )
      call LIS_verify(ios,'Error in nf90_get_var(PRECCON) in genEnsFcst_Variables_read')
      do n = 1, LIS_rc%nnest
         call genEnsFcst_interp_data( n, findex, inc, inr, tindex,&
                 input_var, .true., ensfcstvars_struc(n)%cpcp )
      end do
    endif

!- Close netCDF file.
   ios=nf90_close(nid)
   call LIS_verify(ios,'Error in nf90_close in genEnsFcst_Variables_read')
!-
#endif

 end subroutine genEnsFcst_Variables_read

!
!  subroutine genEnsFcst_Variables_reset( n, findex )
!
!  end subroutine genEnsFcst_Variables_reset

!
!  subroutine genEnsFcst_Variables_final( n, findex )
!
!  end subroutine genEnsFcst_Variables_final

end module genEnsFcst_VariablesMod

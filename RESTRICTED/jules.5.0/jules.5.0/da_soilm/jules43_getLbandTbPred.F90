...
!BOP
! !ROUTINE: jules43_getLbandTbPred
! \label{jules43_getLbandTbPred}
!
! !REVISION HISTORY:
! 13 Sept 2012: Sujay Kumar; Initial Specification
!
! !INTERFACE:
subroutine jules43_getLbandTbPred(n, obs_pred)
! !USES:
  use ESMF
...
!call the forward model
  ...
  call LIS_verify(status, &
       "Error in StateGet in jules43_getLbandTbPred for Lband_Hpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbH, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in jules43_getLbandTbPred for Lband_Hpol')

  call ESMF_StateGet(LIS_forwardState(n), "Lband_Vpol", varField, rc=status)
  call LIS_verify(status, &
       "Error in StateGet in jules43_getLbandTbPred for Lband_Vpol")

  call ESMF_FieldGet(varField, localDE=0,farrayPtr=TbV, rc=status)
  call LIS_verify(status, &
       'Error in FieldGet in jules43_getLbandTbPred for Lband_Vpol')
...
end subroutine jules43_getLbandTbPred

...
!BOP
!
! !ROUTINE: jules50_initialize
! \label{jules50_initialize}
!
! !REVISION HISTORY:
!
! !INTERFACE:
subroutine jules50_initialize(mtype)
   ! !USES:
...
! !DESCRIPTION:
!
!  This routine initializes the jules50 state variables with some
...
!EOP
   ...
   do n=1, LIS_rc%nnest
      write(LIS_logunit,*) "MSG: jules50_initialize -- initialzie for jules50"
      k = 1
      do
         ...
         do t=start_k, end_k
           ...
           bexp_soilt(1,1,:)   = jules50_struc(n)%jules50(t)%b
           sathh_soilt(1,1,:)  = jules50_struc(n)%jules50(t)%sathh
           satcon_soilt(1,1,:) = jules50_struc(n)%jules50(t)%satcon
           smvcst_soilt(1,1,:) = jules50_struc(n)%jules50(t)%sm_sat
           smvccl_soilt(1,1,:) = jules50_struc(n)%jules50(t)%sm_crit
           smvcwt_soilt(1,1,:) = jules50_struc(n)%jules50(t)%sm_wilt
           hcap_soilt(1,1,:)   = jules50_struc(n)%jules50(t)%hcap
           hcon_soilt(1,1,:)   = jules50_struc(n)%jules50(t)%hcon
           albsoil_soilt(1,1)  = jules50_struc(n)%jules50(t)%albsoil 
         ...
         call init_ic(jules50_struc(n)%namelist_dir) 
         call init_params(jules50_struc(n)%namelist_dir)
         call init_parms()

...
! 4. irrigation
  ...
  ! SY: Begin moving to here from NOAHMP_VEG_PARAMETERS_36 to facilitate OPTUE
  ...
  ! SY: End moving to here from NOAHMP_VEG_PARAMETERS_36 to facilitate OPTUE


!------------------------------------------------------------------------------------------!
END MODULE NOAHMP_GLOBALS_36
!------------------------------------------------------------------------------------------!
!------------------------------------------------------------------------------------------!
MODULE NOAHMP_VEG_PARAMETERS_36

    USE NOAHMP_GLOBALS_36, ONLY: MVT, MBAND,  &
          CH2OP, DLEAF, Z0MVT, HVT, HVB, RC, &
          SLA, DILEFC, DILEFW, FRAGR, LTOVRC, &
          C3PSN, KC25, AKC, KO25, AKO, VCMX25, &
          AVCMX, BP, MP, QE25, AQE, RMF25, RMS25, &
          RMR25, ARM, FOLNMX, TMIN, XL, RHOL, RHOS, &
          TAUL, TAUS, MRP, CWPVT, WRRAT, WDPOOL, TDLEF 

    IMPLICIT NONE

    INTEGER, PARAMETER :: MAX_VEG_PARAMS = 33
    ! SY: Begin commenting out and moving to NOAHMP_GLOBALS_36 and using from there to facilitate OPTUE
!    INTEGER, PARAMETER :: MVT   = 27
!    INTEGER, PARAMETER :: MBAND = 2
    ! SY: End commenting out and moving to NOAHMP_GLOBALS_36 and using from there to facilitate OPTUE
    ...
    ! SY: Begin commenting out some variables and moving to NOAHMP_GLOBALS_36 and using from there to facilitate OPTUE
!    REAL :: CH2OP(MVT)       !maximum intercepted h2o per unit lai+sai (mm)
!    REAL :: DLEAF(MVT)       !characteristic leaf dimension (m)
!    REAL :: Z0MVT(MVT)       !momentum roughness length (m)
!    REAL :: HVT(MVT)         !top of canopy (m)
...
!    REAL :: WDPOOL(MVT)      !wood pool (switch 1 or 0) depending on woody or not [-]
!    REAL :: TDLEF(MVT)       !characteristic T for leaf freezing [K]
    ! SY: End commenting out some variables and moving to NOAHMP_GLOBALS_36 and using from there to facilitate OPTUE
...
CONTAINS
  subroutine read_mp_veg_parameters(MPTABLE_NAME, DATASET_IDENTIFIER)
    implicit none
    character(len=*), intent(in) :: MPTABLE_NAME
    ...
    NAMELIST / noah_mp_umd_veg_categories / VEG_DATASET_DESCRIPTION, NVEG
    NAMELIST / noah_mp_umd_parameters / ISURBAN, ISWATER, ISBARREN, ISSNOW, EBLFOREST, &
         CH2OP, DLEAF, Z0MVT, HVT, HVB, DEN, RC, RHOL,  RHOS, TAUL, TAUS, XL, CWPVT, C3PSN, KC25, AKC, KO25, AKO, AVCMX, AQE, &
         LTOVRC,  DILEFC,  DILEFW,  RMF25 ,  SLA   ,  FRAGR ,  TMIN  ,  VCMX25,  TDLEF ,  BP, MP, QE25, RMS25, RMR25, ARM, &
         FOLNMX, WDPOOL, WRRAT, MRP, SAIM,  LAIM,  SLAREA, EPS
    ...
    open(15, file=trim(MPTABLE_NAME), status='old', form='formatted', action='read', iostat=ierr)
    if (ierr /= 0) then
       write(*,"('****** Error ******************************************************')")
    ...
    else if ( trim(DATASET_IDENTIFIER) == "UMD" ) then
       read(15,noah_mp_umd_veg_categories)
       read(15,noah_mp_umd_parameters)
    else
...
END MODULE NOAHMP_VEG_PARAMETERS_36
! ==================================================================================================
! ==================================================================================================
MODULE NOAHMP_RAD_PARAMETERS_36
...
END MODULE NOAHMP_RAD_PARAMETERS_36
! ==================================================================================================

MODULE NOAHMP_ROUTINES_36
  USE NOAHMP_GLOBALS_36
  IMPLICIT NONE

  public  :: noahmp_options
  public  :: NOAHMP_SFLX_36
  public  :: REDPRM
  public  :: FRH2O           
  public  :: COMPACT
  public  :: COMBINE
...
! vegetation phenology
     ...
     CALL PHENOLOGY (iloc,jloc,VEGTYP , ISURBAN, SNOWH  , TV     , LAT   , YEARLEN , JULIAN , & !in
...
! compute energy budget (momentum & energy fluxes and phase changes) 
    CALL ENERGY (UPD_ALB_FLAG,ALBD_IN, ALBI_IN, &
...
! ==================================================================================================
! --------------------------------------------------------------------------------------------------
  SUBROUTINE PHENOLOGY (iloc,jloc,VEGTYP , ISURBAN, SNOWH  , TV     , LAT   , YEARLEN , JULIAN , & !in
                        LAI    , SAI    , TROOT  , HTOP  , ELAI    , ESAI   , IGS)
...
! ==================================================================================================
! --------------------------------------------------------------------------------------------------
  SUBROUTINE ENERGY (UPD_ALB_FLAG,ALBD_IN, ALBI_IN, &
                     ICE    ,VEGTYP ,IST    ,ISC    ,NSNOW  ,NSOIL  , & !in
                     ISNOW  ,NROOT  ,DT     ,RHOAIR ,SFCPRS ,QAIR   , & !in
...
! inputs
  LOGICAL                              :: UPD_ALB_FLAG
  REAL, DIMENSION(1:2)                 :: ALBD_IN !ground albedo (direct)
  REAL, DIMENSION(1:2)                 :: ALBI_IN !ground albedo (diffuse)
...
! Solar radiation: absorbed & reflected by the ground and canopy
  ...
  CALL  RADIATION (UPD_ALB_FLAG,ALBD_IN, ALBI_IN, & 
...
! UNFROZEN VOLUME FOR SATURATION (POROSITY*XUNFROZ)
    if (SMC.eq.0.0) then  ! prevent divide-by-zero [David Mocko]
       XUNFROZ = 1.0
    else
       XUNFROZ = SH2O / SMC
    endif
...
! ==================================================================================================
  SUBROUTINE RADIATION (UPD_ALB_FLAG,ALBD_IN, ALBI_IN,&
                        VEGTYP  ,IST     ,ISC     ,ICE     ,NSOIL   , & !in
                        SNEQVO  ,SNEQV   ,DT      ,COSZ    ,SNOWH   , & !in
                        TG      ,TV      ,FSNO    ,QSNOW   ,FWET    , & !in
                        ELAI    ,ESAI    ,SMC     ,SOLAD   ,SOLAI   , & !in
...
!jref:end
  ...
  REAL                                 :: ALBOLD_HOLD
  REAL                                 :: TAUSS_HOLD
  REAL, DIMENSION(1:2)                 :: ALBD_F
  REAL, DIMENSION(1:2)                 :: ALBI_F
  INTEGER                              :: IB
...
!This is a DA related update: This code block is necessary because
!albedo variables are not exposed. 
     IF(UPD_ALB_FLAG) then 
        ALBOLD = ALBOLD_HOLD
        TAUSS  = TAUSS_HOLD
        
        IF(ALBGRD(1).ne.0) then 
           ALBD_F(1) = ALBD(1)/ALBGRD(1)
        ELSE
           ALBD_F(1) = 0.0
        ENDIF
        IF(ALBGRI(1).NE.0) THEN 
           ALBI_F(1) = ALBI(1)/ALBGRI(1)
        ELSE
           ALBI_F(1) = 0.0
        ENDIF
        IF(ALBGRD(2).NE.0) THEN 
           ALBD_F(2) = ALBD(2)/ALBGRD(2)
        ELSE
           ALBD_F(2) = 0.0
        ENDIF
        IF(ALBGRI(2).NE.0) THEN 
           ALBI_F(2) = ALBI(2)/ALBGRI(2)
        ELSE
           ALBI_F(2) = 0.0
        ENDIF
!Compute the ground albedos using the same factor used in the model
        DO IB=1,2
           ALBD(IB) = ALBD_IN(IB)
           ALBI(IB) = ALBI_IN(IB)

           ALBGRD(IB) = ALBD_IN(IB)*ALBD_F(IB)
           ALBGRI(IB) = ALBI_IN(IB)*ALBI_F(IB)
        ENDDO

! surface abeldo

        CALL ALBEDO_UPD (VEGTYP ,IST    ,ISC    ,ICE    ,NSOIL  , & !in
             DT     ,COSZ   ,FAGE   ,ELAI   ,ESAI   , & !in
             TG     ,TV     ,SNOWH  ,FSNO   ,FWET   , & !in
             SMC    ,SNEQVO ,SNEQV  ,QSNOW  ,FVEG   , & !in
             ILOC   ,JLOC   ,                         & !in
...
100 CONTINUE
  ...
  SUBROUTINE ALBEDO_UPD (VEGTYP ,IST    ,ISC    ,ICE    ,NSOIL  , & !in
                     DT     ,COSZ   ,FAGE   ,ELAI   ,ESAI   , & !in
                     TG     ,TV     ,SNOWH  ,FSNO   ,FWET   , & !in
                     SMC    ,SNEQVO ,SNEQV  ,QSNOW  ,FVEG   , & !in
                     ILOC   ,JLOC   ,                         & !in
...
! Added by David Mocko to only do CAH2 calculation
! for SFC option 1, as FH2 is undefined otherwise.
      IF (OPT_SFC == 1) THEN
...
! -----------------------------------------------------------------
        ...
        QFX    = 0.
        FV     = 0.1
        MOZ = 0.  ! kwh ken added 02/12/2016
...
! Added by David Mocko to only do EHB2 calculation
! for SFC option 1, as FH2 is undefined otherwise.
       IF (OPT_SFC == 1) THEN
...
! local
  ...
  REAL                            :: BDSNOW    !add Yeosang, temporary variable for snow density
...
! The rate of melting and freezing for snow without a layer, needs more work.
    ...
    IF (ISNOW == 0 .AND. SNEQV > 0. .AND. XM(1) > 0.) THEN  
        ...
        PROPOR = (SNEQV-TEMP1)/BDSNOW            ! Yeosang, 2018,08,23 code from Niu, Guo-Yue
        SNOWH  = MAX(0.,SNOWH + PROPOR)
...
! output
  ...
  REAL,                            INTENT(OUT)   :: FPICE   !snow fraction in precipitation
  REAL,                            INTENT(OUT)   :: SUBSNOW   !snow fraction in precipitation
...
!     Yeosang Yoon,  2017/08/18: fix no snow layer issues (thin snow), codes from Niu, Guo-Yue 
...
! MB: do each if block separately
   ...
   IF(ISNOW < 0) then        !when multi-layer
...
!     Yeosang Yoon,  2017/08/18: fix no snow layer issues (thin snow), codes from Niu, Guo-Yue
   IF(ISNOW == 0 .and. SNEQV > 0. .and. SNOWH > 0.) THEN
      ...
      SNEQV  = max(0., SNEQV - QSNSUB*DT + QSNFRO*DT) ! changed Aug. 28, 2017 - Yeosang
      !PROPOR = SNEQV/TEMP
      !SNOWH  = MAX(0.,PROPOR * SNOWH)
      PROPOR = (SNEQV-TEMP)/BDSNOW
      SNOWH  = MAX(0.,SNOWH + PROPOR)
...
!      IF (J >= ISNOW+1) THEN
!         SNLIQ(J) = SNLIQ(J) + QIN
!!         VOL_LIQ(J) = SNLIQ(J)/(DZSNSO(J)*DENH2O)   !added by niu
!
!         IF (J <= -1) THEN
...
!   END DO
    ...
    ! Revise codes, Yeosang Yoon - 08/15/2017, Niu, Guo-Yue
    ! This can effectively force the excessive liquid water in the surface 
    ! layer to flow down to lower layers and the surface soil.
    DO J = ISNOW+1, 0
             VOL_ICE(J)      = MIN(1., SNICE(J)/(DZSNSO(J)*DENICE))
             EPORE(J)        = 1. - VOL_ICE(J)
    END DO
...
! constants
    ...
    WSTRC   = 100.0
    LAIMIN  = 0.05   
    XSAMIN  = 0.05          ! Modified by Shugong Wang for dynamic vegetation
...
! Changing IF statement value to prevent floating overflow errors. -
! David Mocko
!     IF(WOOD.GT.0) THEN
     IF(WOOD.GT.1.e-6) THEN
...
! ------------------------------------------------------------------------------------------
  SUBROUTINE BVOCFLUX(VOCFLX,  VEGTYP,  VEGFRAC,  APAR,   TV )
    use NOAHMP_VEG_PARAMETERS_36 , ONLY : SLAREA, EPS
! ------------------------------------------------------------------------------------------
...
END MODULE NOAHMP_ROUTINES_36
! ==================================================================================================

MODULE MODULE_SF_NOAHMPLSM_36

  USE NOAHMP_ROUTINES_36
  USE NOAHMP_GLOBALS_36
  USE NOAHMP_VEG_PARAMETERS_36

END MODULE MODULE_SF_NOAHMPLSM_36

...
!  use module_sf_noah33lsm, only : SFLX, DQSDT, SFCDIF_OFF, CALHUM, SNFRAC
  use module_sf_noah33lsm, only : SFLX, SFCDIF_OFF, CALHUM, SNFRAC
  use module_sfcdif_wrf_33, only: SFCDIF_MYJ
  use module_sf_noah33lsm_glacial, only : SFLX_glacial
  use noah33_lsmMod
...
!
! !DESCRIPTION:
!  This is the entry point for calling the Noah3.3 LSM physics.
!  This routine calls the {\tt SFLX} routine that performs the
!  land surface computations, to solve for water and energy equations.
!  For documentation of the {\tt SFLX} and other Noah3.3 routines,
!  please see the Noah3.3 webpage at NCAR:
...
!                           ! diagnosing the mixing ratio at 2 m for coupled model
  ...
  real        :: relsmc   ! Volumetric relative soil moisture (m^3/m^3) (smc - smcwlt) / (porosity - smcwlt)
  integer     :: t,i,k
  real        :: wchange_prev

  REAL, PARAMETER:: R = 287.04
! Noah3.3's value for CP is 1004.6 - D. Mocko
...
! Noah3.3 uses this value instead of 273.16 - D. Mocko
...
! J.Case -- end mod (9/11/2014)
  ...
  write(fnest,'(i3.3)') n
  alarmCheck = LIS_isAlarmRinging(LIS_rc,"Noah33 model alarm "//trim(fnest))
  if(alarmCheck) then
...
!$OMP DO
     do t = 1,LIS_rc%npatch(n,LIS_rc%lsm_index)
        ...
        sfctmp = noah33_struc(n)%noah(t)%tair/noah33_struc(n)%forc_count
        q2     = noah33_struc(n)%noah(t)%qair/noah33_struc(n)%forc_count
        soldn  = noah33_struc(n)%noah(t)%swdown/noah33_struc(n)%forc_count
        lwdn   = noah33_struc(n)%noah(t)%lwdown*&
                 noah33_struc(n)%noah(t)%emiss/noah33_struc(n)%forc_count
        uwind  = (noah33_struc(n)%noah(t)%uwind)*(noah33_struc(n)%noah(t)%uwind)
        vwind  = (noah33_struc(n)%noah(t)%vwind)*(noah33_struc(n)%noah(t)%vwind)
        sfcspd = sqrt( uwind + vwind )/noah33_struc(n)%forc_count
        sfcprs = noah33_struc(n)%noah(t)%psurf/noah33_struc(n)%forc_count
        prcp   = noah33_struc(n)%noah(t)%rainf/noah33_struc(n)%forc_count
        cpcp   = noah33_struc(n)%noah(t)%rainf_c/noah33_struc(n)%forc_count

        if (noah33_struc(n)%noah(t)%q1.lt.0.0) then
           noah33_struc(n)%noah(t)%q1 = q2
        endif
        if((SFCTMP.eq.LIS_rc%udef).or.&
           ...
           write(LIS_logunit,*) '[ERR] Undefined forcing values found in Noah 3.3'
           write(LIS_logunit,*) '[ERR] for tile: ',t, &
           ...
           write(LIS_logunit,*) "[ERR] ... Forcing values at this point: ",&
...
! reproduce the output of the testcase. - dmm
        ...
        ! using the routine and code from the Noah3.3 released code - D. Mocko
        CALL CALHUM(SFCTMP, SFCPRS, Q2SAT, DQSDT2) ! Returns Q2SAT, DQSDT2

        ! Set the heights of the forcing variables (J. Santanello; D. Mocko)
        if ((noah33_struc(n)%zh.lt.0.0).or.(noah33_struc(n)%zm.lt.0.0)) then
           noah33_struc(n)%zh = noah33_struc(n)%noah(t)%fheight
           noah33_struc(n)%zm = noah33_struc(n)%noah(t)%fheight
           if (noah33_struc(n)%noah(t)%fheight.le.0.0) then
              write(LIS_logunit,*) '[ERR] Forcing height less than or'
              write(LIS_logunit,*) '[ERR] equal to zero!  Stopping run'
              call LIS_endrun()
           endif
        endif
        ZLVL = noah33_struc(n)%zh
        ZLVL_WIND = noah33_struc(n)%zm
#else
        Q2SAT = noah33_struc(n)%noah(t)%q2sat
        ZLVL = 0.5*noah33_struc(n)%noah(t)%z
        ZLVL_WIND = 0.5*noah33_struc(n)%noah(t)%z
#endif
        ...
        t1v  =  noah33_struc(n)%noah(t)%t1 * (1.0 + 0.61 * q2)
        ...
        !-------------------------------------------------------------------------
        nsoil = noah33_struc(n)%nslay
        allocate(sldpth(nsoil))
        if(noah33_struc(n)%usedsoilmap.ne.0) then 
           do i=1,nsoil
              sldpth(i) = noah33_struc(n)%noah(t)%lyrthk(i)
           enddo
        else
           do i = 1,nsoil
              sldpth(i) = noah33_struc(n)%lyrthk(i)
           enddo
        endif
        ...
        prcprain = 0.0
        solardirect = 0.0
        ! not used (soil parameters set in noah33_setsoils)
        slope = noah33_struc(n)%noah(t)%slope
        soiltyp = noah33_struc(n)%noah(t)%soiltype

        !-------------------------------------------------------------------------
        ! Maximum Albedo over very Deep Snow
        !-------------------------------------------------------------------------
        snoalb = noah33_struc(n)%noah(t)%mxsnalb
        solnet = soldn*(1.0-noah33_struc(n)%noah(t)%albedo)

        tbot = noah33_struc(n)%noah(t)%tempbot
        ...
        ! then let Noah3.3 calculate its own surface aerodynamic conductance.
        if ((noah33_struc(n)%forcing_ch.eq.0).or.(noah33_struc(n)%forcing_cm.eq.0)) then
           if (noah33_struc(n)%sfcdifoption.eq.0) then
              CALL SFCDIF_OFF(ZLVL,ZLVL_WIND,noah33_struc(n)%noah(t)%z0_old,&
                   T1V,TH2V,SFCSPD,noah33_struc(n)%noah(t)%czil,         &
                   noah33_struc(n)%noah(t)%cm,noah33_struc(n)%noah(t)%ch,&
                   noah33_struc(n)%noah(t)%vegt,ISURBAN,                 &
                   noah33_struc(n)%iz0tlnd,USTAR)
           else if (noah33_struc(n)%sfcdifoption.eq.1) then
              CALL SFCDIF_MYJ(ZLVL,ZLVL_WIND,noah33_struc(n)%noah(t)%z0_old,&
                   noah33_struc(n)%noah(t)%z0brd_old,       &
                   SFCPRS,noah33_struc(n)%noah(t)%t1,              &
                   SFCTMP,noah33_struc(n)%noah(t)%Q1,              &
                   Q2,SFCSPD,noah33_struc(n)%noah(t)%czil,            &
                   RIBB,noah33_struc(n)%noah(t)%cm,              &
                   noah33_struc(n)%noah(t)%ch,              &
                   noah33_struc(n)%noah(t)%vegt,            &
                   ISURBAN,noah33_struc(n)%iz0tlnd,                 &
                   noah33_struc(n)%ztmax2,                  &
                   noah33_struc(n)%dzeta2,                  &
                   noah33_struc(n)%psih2,                   &
                   noah33_struc(n)%psim2,USTAR)
           endif
        else
           ! Still call sfcdif because we need ustar, but do not overwrite
           ! the provided ch and cm values.
           cm = noah33_struc(n)%noah(t)%cm/noah33_struc(n)%forc_count
           ch = noah33_struc(n)%noah(t)%ch/noah33_struc(n)%forc_count
           if (noah33_struc(n)%sfcdifoption.eq.0) then
              CALL SFCDIF_OFF(ZLVL,ZLVL_WIND,noah33_struc(n)%noah(t)%z0_old,&
                   T1V,TH2V,SFCSPD,noah33_struc(n)%noah(t)%czil,cm,ch,   &
                   noah33_struc(n)%noah(t)%vegt,ISURBAN,                 &
                   noah33_struc(n)%iz0tlnd,USTAR)
           else if (noah33_struc(n)%sfcdifoption.eq.1) then
              CALL SFCDIF_MYJ(ZLVL,ZLVL_WIND,noah33_struc(n)%noah(t)%z0_old,&
        ...
        ! Still call sfcdif because we need ustar, but do not overwrite
        ! the provided ch and cm values.
        cm = noah33_struc(n)%noah(t)%cm
        ch = noah33_struc(n)%noah(t)%ch
        if (noah33_struc(n)%sfcdifoption.eq.0) then
!           CALL SFCDIF_OFF(ZLVL,noah33_struc(n)%noah(t)%z0_old,&
!                T1V,TH2V,SFCSPD,noah33_struc(n)%noah(t)%czil,cm,ch,     &
!                noah33_struc(n)%noah(t)%vegt,ISURBAN,     &
!                noah33_struc(n)%iz0tlnd,USTAR)
           CALL SFCDIF_OFF(ZLVL,noah33_struc(n)%noah(t)%z0_old,&
           ...
           CALL SFCDIF_MYJ(ZLVL,noah33_struc(n)%noah(t)%z0_old, &
                noah33_struc(n)%noah(t)%z0brd_old,          &
                SFCPRS,noah33_struc(n)%noah(t)%t1,                 &
                SFCTMP,noah33_struc(n)%noah(t)%Q1,                 &
                Q2,SFCSPD,noah33_struc(n)%noah(t)%czil,               &
        ...
        noah33_struc(n)%noah(t)%kdt = noah33_struc(n)%noah(t)%refkdt* &
             noah33_struc(n)%noah(t)%dksat/&
             noah33_struc(n)%noah(t)%refdk
        frzk = noah33_struc(n)%noah(t)%frzk
        if(noah33_struc(n)%noah(t)%smcref.eq.0) then 
           noah33_struc(n)%noah(t)%frzx = 0.0
        else
           FRZFACT = (noah33_struc(n)%noah(t)%SMCMAX / noah33_struc(n)%noah(t)%SMCREF) &
                * (0.412 / 0.468)
           noah33_struc(n)%noah(t)%FRZX = FRZK * FRZFACT
        endif

        if(noah33_struc(n)%noah(t)%vegt.eq.LIS_rc%waterclass) then
           ice = 1
        else
           ice = 0
        endif
...
!        if(noah33_struc(n)%noah(t)%vegt.eq.LIS_rc%waterclass) then 
!           ! open water case
!           if(noah33_struc(n)%noah(t)%xice.eq.1) then 
!              ice = 1
!              noah33_struc(n)%noah(t)%smc = 1.0
!              noah33_struc(n)%noah(t)%stc = 273.16
!           endif
!        else
        ...
        if(noah33_struc(n)%noah(t)%sneqv.gt.0.0) then
           ! snow on surface (use ice saturation properties)
           call SNFRAC(noah33_struc(n)%noah(t)%sneqv,                  &
                noah33_struc(n)%noah(t)%snup,                   &
                noah33_struc(n)%noah(t)%salp,                   &
                noah33_struc(n)%noah(t)%snowh,SNCOVR)
           ...
           if(noah33_struc(n)%noah(t)%T1.gt.273.15) then
           ...
           if ((noah33_struc(n)%noah(t)%T1.gt.273.).and.               &
        ...
        do i=1,noah33_struc(n)%nslay
           soilmtc = soilmtc +                                              &
                noah33_struc(n)%noah(t)%smc(i)*LIS_CONST_RHOFW*sldpth(i)
        enddo
        ! First attempt at calculating DelSurfHeat - D. Mocko
        !   soilhtc = (((NOAH33_STRUC(N)%NOAH(T)%SH2O(1)*CH2O)                  &
        !             + ((1.0-noah33_struc(n)%noah(t)%SMCMAX)*CSOIL)            &
        !             + ((noah33_struc(n)%noah(t)%SMCMAX-NOAH33_STRUC(N)%NOAH(T)%SMC(1))*CAIR) &
        !             + ((NOAH33_STRUC(N)%NOAH(T)%SMC(1)-NOAH33_STRUC(N)%NOAH(T)%SH2O(1))*CICE)) &
        !             * noah33_struc(n)%noah(t)%stc(1) * SLDPTH(1))
        soilhtc = 0.0
        startht = soilhtc
        startsm = soilmtc
        startswe = noah33_struc(n)%noah(t)%sneqv*LIS_CONST_RHOFW
        startint = noah33_struc(n)%noah(t)%cmc*LIS_CONST_RHOFW
...
#define _DEBUG_PRINT_ 0
        ...
        print*, 'b  VEGTYP = ', noah33_struc(n)%noah(t)%vegt
        print*, 'b  SOILTYP = ', SOILTYP
        print*, 'b  SLOPETYP = ',1
        print*, 'b  SHDFAC = ', noah33_struc(n)%noah(t)%shdfac
        print*, 'b  SHDMIN = ', noah33_struc(n)%noah(t)%shdmin
        print*, 'b  SHDMAX = ', noah33_struc(n)%noah(t)%shdmax
        print*, 'b  ALB = ', noah33_struc(n)%noah(t)%alb
        print*, 'b  SNOALB = ', SNOALB
        print*, 'b  TBOT = ', TBOT
        print*, 'b  Z0BRD = ', noah33_struc(n)%noah(t)%z0brd
        print*, 'b  Z0 = ', noah33_struc(n)%noah(t)%z0
        print*, 'b  EMISSI = ', noah33_struc(n)%noah(t)%emiss
        print*, 'b  EMBRD = ', noah33_struc(n)%noah(t)%embrd
        print*, 'b  CMC = ', noah33_struc(n)%noah(t)%cmc
        print*, 'b  T1 = ', noah33_struc(n)%noah(t)%t1
        print*, 'b  STC = ', noah33_struc(n)%noah(t)%stc
        print*, 'b  SMC = ', noah33_struc(n)%noah(t)%smc
        print*, 'b  SH2O = ', noah33_struc(n)%noah(t)%sh2o
        print*, 'b  SNOWH = ', noah33_struc(n)%noah(t)%snowh
        print*, 'b  SNEQV = ', noah33_struc(n)%noah(t)%sneqv
        print*, 'b  ALBEDO = ', noah33_struc(n)%noah(t)%albedo
        print*, 'b  CH = ', noah33_struc(n)%noah(t)%ch
        print*, 'b  CM = ', noah33_struc(n)%noah(t)%cm
        print*, 'b  ETA = ', ETA
        ...
        print*, 'b  RUNOFF1 = ', noah33_struc(n)%noah(t)%RUNOFF1
        print*, 'b  RUNOFF2 = ', noah33_struc(n)%noah(t)%RUNOFF2
        print*, 'b  RUNOFF3 = ', RUNOFF3
        print*, 'b  RC = ', RC
        print*, 'b  PC = ', PC
        print*, 'b  RSMIN = ', noah33_struc(n)%noah(t)%rsmin
        print*, 'b  XLAI = ', noah33_struc(n)%noah(t)%lai
        print*, 'b  RCS = ', RCS
        print*, 'b  RCT = ', RCT
        print*, 'b  RCQ = ', RCQ
        ...
        print*, 'b  SOILW = ', SOILW
        print*, 'b  SOILM = ', SOILM
        print*, 'b  Q1 = ', noah33_struc(n)%noah(t)%Q1
#if 0
        print*, 'b  RDLAI2D = ', RDLAI2D
        print*, 'b  USEMONALB = ', USEMONALB
#endif
        print*, 'b  SNOTIME1 = ', noah33_struc(n)%noah(t)%snotime1
        print*, 'b  RIBB = ', RIBB
        print*, 'b  SMCWLT = ', noah33_struc(n)%noah(t)%smcwlt
        print*, 'b  SMCDRY = ', noah33_struc(n)%noah(t)%smcdry
...
!           print*, 'main1 ',t,noah33_struc(n)%noah(t)%smc(1),&
!                noah33_struc(n)%noah(t)%smc(2),&
!                noah33_struc(n)%noah(t)%smc(3),&
!                noah33_struc(n)%noah(t)%smc(4)
!        endif
!     endif
        if(ice.eq.0) then 
           ...
           call sflx(                                                                 &
                ffrozp, ice, isurban, dt, zlvl, nsoil, sldpth,                        &
                local,                                                                &
                llanduse, lsoil,                                                      & 
                lwdn, soldn, solnet, sfcprs, prcp, sfctmp, q2, sfcspd,                &
                cosz, prcprain, solardirect,                                          & 
                th2, q2sat, dqsdt2,                                                   & 
                noah33_struc(n)%noah(t)%vegt, soiltyp, slope, & 
                noah33_struc(n)%noah(t)%shdfac, noah33_struc(n)%noah(t)%shdmin,       & 
                noah33_struc(n)%noah(t)%shdmax, & 
                noah33_struc(n)%noah(t)%alb, snoalb, tbot,noah33_struc(n)%noah(t)%z0brd, & 
                noah33_struc(n)%noah(t)%z0, noah33_struc(n)%noah(t)%emiss,            & 
                noah33_struc(n)%noah(t)%embrd,    &
                noah33_struc(n)%noah(t)%cmc, noah33_struc(n)%noah(t)%t1,              & 
                noah33_struc(n)%noah(t)%stc, noah33_struc(n)%noah(t)%smc,             & 
                noah33_struc(n)%noah(t)%sh2o, noah33_struc(n)%noah(t)%snowh,          & 
                noah33_struc(n)%noah(t)%sneqv, noah33_struc(n)%noah(t)%albedo,        & 
                noah33_struc(n)%noah(t)%ch, noah33_struc(n)%noah(t)%cm,               & 
                ...
                noah33_struc(n)%noah(t)%smcwlt, noah33_struc(n)%noah(t)%smcdry,       & 
                noah33_struc(n)%noah(t)%smcref, noah33_struc(n)%noah(t)%smcmax,       &
                noah33_struc(n)%noah(t)%nroot,                                   & 
                noah33_struc(n)%noah(t)%hs, noah33_struc(n)%noah(t)%rgl,         & 
                noah33_struc(n)%noah(t)%snup,                                    & 
                noah33_struc(n)%noah(t)%cmcmax, noah33_struc(n)%noah(t)%rsmax,   & 
                noah33_struc(n)%noah(t)%topt, noah33_struc(n)%noah(t)%bexp,      & 
                noah33_struc(n)%noah(t)%dksat, noah33_struc(n)%noah(t)%dwsat,    & 
                noah33_struc(n)%noah(t)%f1, noah33_struc(n)%noah(t)%quartz,      & 
                noah33_struc(n)%noah(t)%psisat, noah33_struc(n)%noah(t)%czil,    & 
                noah33_struc(n)%noah(t)%sbeta, noah33_struc(n)%noah(t)%fxexp,    & 
                noah33_struc(n)%noah(t)%csoil, noah33_struc(n)%noah(t)%salp,     & 
                noah33_struc(n)%noah(t)%kdt, noah33_struc(n)%noah(t)%cfactr, & 
...
!     if(LIS_localPet.eq.8) then 
!        if(t.ge.61.and.t.le.80) then 
!           print*, 'main2 ',t,noah33_struc(n)%noah(t)%smc(1),&
!                noah33_struc(n)%noah(t)%smc(2),&
!                noah33_struc(n)%noah(t)%smc(3),&
!                noah33_struc(n)%noah(t)%smc(4)
!        endif
!     endif
        ...
        print*, 'a  VEGTYP = ', noah33_struc(n)%noah(t)%vegt
        print*, 'a  SOILTYP = ', SOILTYP
        print*, 'a  SLOPETYP = ',1
        print*, 'a  SHDFAC = ', noah33_struc(n)%noah(t)%shdfac
        print*, 'a  SHDMIN = ', noah33_struc(n)%noah(t)%shdmin
        print*, 'a  SHDMAX = ', noah33_struc(n)%noah(t)%shdmax
        print*, 'a  ALB = ', noah33_struc(n)%noah(t)%alb
        print*, 'a  SNOALB = ', SNOALB
        print*, 'a  TBOT = ', TBOT
        print*, 'a  Z0BRD = ', noah33_struc(n)%noah(t)%z0brd
        print*, 'a  Z0 = ', noah33_struc(n)%noah(t)%z0
        print*, 'a  EMISSI = ', noah33_struc(n)%noah(t)%emiss
        print*, 'a  EMBRD = ', noah33_struc(n)%noah(t)%embrd
        print*, 'a  CMC = ', noah33_struc(n)%noah(t)%cmc
        print*, 'a  T1 = ', noah33_struc(n)%noah(t)%t1
        print*, 'a  STC = ', noah33_struc(n)%noah(t)%stc
        print*, 'a  SMC = ', noah33_struc(n)%noah(t)%smc
        print*, 'a  SH2O = ', noah33_struc(n)%noah(t)%sh2o
        print*, 'a  SNOWH = ', noah33_struc(n)%noah(t)%snowh
        print*, 'a  SNEQV = ', noah33_struc(n)%noah(t)%sneqv
        print*, 'a  ALBEDO = ', noah33_struc(n)%noah(t)%albedo
        print*, 'a  CH = ', noah33_struc(n)%noah(t)%ch
        print*, 'a  CM = ', noah33_struc(n)%noah(t)%cm
        print*, 'a  ETA = ', ETA
        ...
        print*, 'a  RUNOFF1 = ', noah33_struc(n)%noah(t)%RUNOFF1
        print*, 'a  RUNOFF2 = ', noah33_struc(n)%noah(t)%RUNOFF2
        print*, 'a  RUNOFF3 = ', RUNOFF3
        print*, 'a  RC = ', RC
        print*, 'a  PC = ', PC
        print*, 'a  RSMIN = ', noah33_struc(n)%noah(t)%rsmin
        print*, 'a  XLAI = ', noah33_struc(n)%noah(t)%lai
        print*, 'a  RCS = ', RCS
        print*, 'a  RCT = ', RCT
        print*, 'a  RCQ = ', RCQ
        ...
        print*, 'a  SOILW = ', SOILW
        print*, 'a  SOILM = ', SOILM
        print*, 'a  Q1 = ', noah33_struc(n)%noah(t)%Q1
#if 0
        print*, 'a  RDLAI2D = ', RDLAI2D
        print*, 'a  USEMONALB = ', USEMONALB
#endif
        print*, 'a  SNOTIME1 = ', noah33_struc(n)%noah(t)%snotime1
        print*, 'a  RIBB = ', RIBB
        print*, 'a  SMCWLT = ', noah33_struc(n)%noah(t)%smcwlt
        print*, 'a  SMCDRY = ', noah33_struc(n)%noah(t)%smcdry
        ...
        elseif(ice.eq.1) then 
           ...
           noah33_struc(n)%noah(t)%lai = 0.01
        elseif(ice.eq.-1) then 
           SOILM = 0.0 !BSINGH(PNNL)- SOILM is undefined for this case, it is used for diagnostics so setting it to zero
           noah33_struc(n)%noah(t)%lai = 0.01 
           noah33_struc(n)%noah(t)%RUNOFF2 = 0.0
           RUNOFF3 = 0.0
           noah33_struc(n)%noah(t)%smc = 1.0
           noah33_struc(n)%noah(t)%sh2o = 1.0

           CALL SFLX_GLACIAL(ICE, & 
                FFROZP,DT,ZLVL,NSOIL,SLDPTH,   &   
                LWDN,SOLNET,SFCPRS,PRCP,SFCTMP,Q2,&
                TH2,Q2SAT,DQSDT2,                                &   
                noah33_struc(n)%noah(t)%alb, &
                SNOALB,TBOT, noah33_struc(n)%noah(t)%Z0BRD, noah33_struc(n)%noah(t)%Z0, &
                noah33_struc(n)%noah(t)%EMISS, noah33_struc(n)%noah(t)%EMBRD, &  
                noah33_struc(n)%noah(t)%T1,noah33_struc(n)%noah(t)%STC,&
                noah33_struc(n)%noah(t)%SNOWH,noah33_struc(n)%noah(t)%SNEQV,&
                noah33_struc(n)%noah(t)%ALBEDO,noah33_struc(n)%noah(t)%CH,        &   
                ETA,SHTFLX,ETA_KINEMATIC,FDOWN,                   &   
                ESNOW,DEW,                                       &   
                ETP,GFLX,                                       &   
                FLX1,FLX2,FLX3,                                  &   
                SNOMLT,SNCOVR,                                   &   
                noah33_struc(n)%noah(t)%RUNOFF1,                                         &   
                noah33_struc(n)%noah(t)%Q1,                                              &   
                noah33_struc(n)%noah(t)%SNOTIME1,                                        &
                RIBB)
           
        endif
        noah33_struc(n)%noah(t)%qh = shtflx
        noah33_struc(n)%noah(t)%qg = gflx
        noah33_struc(n)%noah(t)%qle = eta
        noah33_struc(n)%noah(t)%eta_kinematic = eta_kinematic

! J.Case (9/11/2014) -- This block of code should be done only in coupled runs.
#if (defined COUPLED)
        noah33_struc(n)%noah(t)%chs2 = noah33_struc(n)%noah(t)%cqs2
! J.Case (9/11/2014) -- Moved 2 lines below to ensure qsfc is defined before the if-block after the line
        noah33_struc(n)%noah(t)%qsfc = noah33_struc(n)%noah(t)%q1/&
             (1-noah33_struc(n)%noah(t)%q1)
        if (noah33_struc(n)%noah(t)%q1.gt.noah33_struc(n)%noah(t)%qsfc) then 
            noah33_struc(n)%noah(t)%cqs2 = noah33_struc(n)%noah(t)%chs2
        endif
#endif
! J.Case (9/11/2014, end mods)

        if (SFCTMP .lt. noah33_struc(n)%noah(t)%tair_agl_min) then
           noah33_struc(n)%noah(t)%tair_agl_min = SFCTMP
           noah33_struc(n)%noah(t)%rhmin = Q2/Q2SAT
        endif

        noah33_struc(n)%noah(t)%soilm = soilm*1000.0
        noah33_struc(n)%noah(t)%qs    = noah33_struc(n)%noah(t)%runoff1 *  &
                                        LIS_CONST_RHOFW
        noah33_struc(n)%noah(t)%qsb   = noah33_struc(n)%noah(t)%runoff2 *  &
                                        LIS_CONST_RHOFW
        noah33_struc(n)%noah(t)%qsm   = snomlt*LIS_CONST_RHOFW/dt

        call LIS_diagnoseSurfaceOutputVar(n, t,LIS_MOC_SWNET,value=soldn * &
             (1.0-noah33_struc(n)%noah(t)%albedo),vlevel=1,unit="W m-2",&
             direction="DN",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n, t,LIS_MOC_LWNET,vlevel=1,  &
             value=(-1.0*(noah33_struc(n)%noah(t)%emiss*                &
             ((5.67E-8)*(NOAH33_STRUC(N)%NOAH(T)%T1**4.0))-             &
             (noah33_struc(n)%noah(t)%lwdown*                           &
             noah33_struc(n)%noah(t)%emiss))),unit="W m-2",&
             direction="DN",surface_type=LIS_rc%lsm_index)
        ...
        call LIS_diagnoseSurfaceOutputVar(n, t,LIS_MOC_QTAU,                     &
             value=(SFCPRS/(R*T2V)*noah33_struc(n)%noah(t)%CM*USTAR),     &
        ...
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_RAINFCONV,value=cpcp,      &
             vlevel=1,unit="kg m-2 s-1",direction="DN",&
             surface_type=LIS_rc%lsm_index,valid_min=0.0,valid_max=0.02)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_RAINFCONV,value=cpcp*dt,   &
             vlevel=1,unit="kg m-2",direction="DN",surface_type=LIS_rc%lsm_index)
...
!             value=prcp*dt,vlevel=1,unit="kg m-2",direction="DN",surface_type=LIS_rc%lsm_index)
        ...
        noah33_struc(n)%noah(t)%evap = evp
        ...
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_QS,                        &
             value= noah33_struc(n)%noah(t)%runoff1*LIS_CONST_RHOFW,      &
             vlevel=1,unit="kg m-2 s-1",direction="OUT",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_QS,                        &
             value= noah33_struc(n)%noah(t)%runoff1*LIS_CONST_RHOFW*dt,   &
             vlevel=1,unit="kg m-2",direction="OUT",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_QSB,                       &
             value= noah33_struc(n)%noah(t)%runoff2*LIS_CONST_RHOFW,      &
             vlevel=1,unit="kg m-2 s-1",direction="OUT",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_QSB,                       &
             value= noah33_struc(n)%noah(t)%runoff2*LIS_CONST_RHOFW*dt,   &
             vlevel=1,unit="kg m-2",direction="OUT",surface_type=LIS_rc%lsm_index)
        ...
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_DELSWE,                    &
             value=(noah33_struc(n)%noah(t)%sneqv*LIS_CONST_RHOFW-        &
        ...
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_DELINTERCEPT,              &
             value=(noah33_struc(n)%noah(t)%cmc*LIS_CONST_RHOFW-startint),&
        ...
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_AVGSURFT,value=(TSOIL *    &
             (1.0 - SNCOVR)) + (NOAH33_STRUC(N)%NOAH(T)%T1 * SNCOVR),     &
             vlevel=1,unit="K",direction="-",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_RADT,                      &
             value=NOAH33_STRUC(N)%NOAH(T)%T1,vlevel=1,unit="K",&
             direction="-",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_ALBEDO,                    &
             value=noah33_struc(n)%noah(t)%albedo,vlevel=1,unit="-",&
             direction="-",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_ALBEDO,                    &
             value=(noah33_struc(n)%noah(t)%albedo*100.),vlevel=1,&
        ...
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_SWE,vlevel=1,unit="m",     &
             value=noah33_struc(n)%noah(t)%sneqv,&
             direction="-",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_SNOWDEPTH,                 &
             vlevel=1,value=noah33_struc(n)%noah(t)%snowh,unit="m",&
             direction="-",surface_type=LIS_rc%lsm_index)
        noah33_struc(n)%noah(t)%sca = sncovr
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_SNOWCOVER,value=SNCOVR,    &
             vlevel=1,unit="-",direction="-",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_RHMIN,                     &
             value=noah33_struc(n)%noah(t)%rhmin,vlevel=1,unit="-",direction="-",&
             surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_RHMIN,                     &
             value=(noah33_struc(n)%noah(t)%rhmin*100.),vlevel=1,unit="%",&
             direction="-",surface_type=LIS_rc%lsm_index)

        do i=1,noah33_struc(n)%nslay
           call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_SOILMOIST,vlevel=i,     &
                value=noah33_struc(n)%noah(t)%smc(i)*sldpth(i)*           &
                LIS_CONST_RHOFW,unit='kg m-2',direction="-",surface_type=LIS_rc%lsm_index)
           call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_SOILMOIST,vlevel=i,     &
                value=noah33_struc(n)%noah(t)%smc(i),unit='m^3 m-3',&
                direction="-",surface_type=LIS_rc%lsm_index)
        enddo
        do i=1,noah33_struc(n)%nslay
           call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_SOILTEMP,vlevel=i,      &
                value=noah33_struc(n)%noah(t)%stc(i),unit="K",&
                direction="-",surface_type=LIS_rc%lsm_index)
        enddo
        do i=1,noah33_struc(n)%nslay
           call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_SMLIQFRAC,vlevel=i,     &
                value=(noah33_struc(n)%noah(t)%sh2o(i)/                   &
                (noah33_struc(n)%noah(t)%sh2o(i) +                 &
                noah33_struc(n)%noah(t)%smc(i))),unit='-',&
                direction="-",surface_type=LIS_rc%lsm_index)
           call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_SMLIQFRAC,vlevel=i,     &
                value=noah33_struc(n)%noah(t)%sh2o(i),unit='m^3 m-3',&
                direction="-",surface_type=LIS_rc%lsm_index)
        enddo

        do i=1,noah33_struc(n)%nslay
           relsmc = &
                (noah33_struc(n)%noah(t)%smc(i) - noah33_struc(n)%noah(t)%smcwlt) / &
                (noah33_struc(n)%noah(t)%smcmax - noah33_struc(n)%noah(t)%smcwlt)
...
! ----------------------------------------------------------------------
     ...
     soiltm = &
       (noah33_struc(n)%noah(t)%smcmax - noah33_struc(n)%noah(t)%smcwlt) * &
        noah33_struc(n)%lyrthk(1)
     soiltw = &
       (noah33_struc(n)%noah(t)%smc(1) - noah33_struc(n)%noah(t)%smcwlt) * &
        noah33_struc(n)%lyrthk(1)
     do i=2,noah33_struc(n)%nslay
       soiltm = soiltm + &
         (noah33_struc(n)%noah(t)%smcmax - noah33_struc(n)%noah(t)%smcwlt) * &
         (noah33_struc(n)%lyrthk(i))
       soiltw = soiltw + &
         (noah33_struc(n)%noah(t)%smc(i) - noah33_struc(n)%noah(t)%smcwlt) * &
         (noah33_struc(n)%lyrthk(i))
     enddo
...
!
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_SOILWET, &
             value=noah33_struc(n)%noah(t)%soilm, &
...
! J.Case (9/11/2014) -- end mods from LIS6/Noah2.7.1
        ...
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_CANOPINT,vlevel=1,         &
             value=noah33_struc(n)%noah(t)%cmc*LIS_CONST_RHOFW,           &
             unit="kg m-2",direction="-",surface_type=LIS_rc%lsm_index)
        ! ROOT ZONE COLUMN SOIL MOISTURE IN METERS (SOILRZ)
        do k = 1,noah33_struc(n)%noah(t)%nroot
           soilrz    = soilrz + (noah33_struc(n)%noah(t)%smc(k)*sldpth(k) *    &
                                 LIS_CONST_RHOFW)
           soilrzmax = soilrzmax + (noah33_struc(n)%noah(t)%smcmax*sldpth(k) * &
                                    LIS_CONST_RHOFW) ! SY
        end do
        noah33_struc(n)%noah(t)%rootmoist = soilrz   !in volumetric
        ...
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_CHFORC,                    &
             value=noah33_struc(n)%noah(t)%ch,vlevel=1,unit="m s-1",&
             direction="-",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_EMISSFORC,                 &
             value=noah33_struc(n)%noah(t)%emiss,vlevel=1,unit="-",&
             direction="-",surface_type=LIS_rc%lsm_index)
! J.Case (9/11/2014) -- Added support for outputting GVF in % for AWIPS II display.
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_GREENNESS,                 &
             value=noah33_struc(n)%noah(t)%shdfac,vlevel=1,unit="-",             &
             direction="-",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_GREENNESS,                 &
             value=noah33_struc(n)%noah(t)%shdfac*100.,vlevel=1,unit="%",        &
             direction="-",surface_type=LIS_rc%lsm_index)

! EMK Bug fix...Output deep soil temperature
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_TEMPBOT,                   &
             value=noah33_struc(n)%noah(t)%tempbot,vlevel=1,unit="K",            &
             direction="-",surface_type=LIS_rc%lsm_index)
          
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_Z0BRD,                     &
             value=noah33_struc(n)%noah(t)%z0brd,vlevel=1,unit="m",&
             direction="-",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_ROUGHNESS,                 &
             value=noah33_struc(n)%noah(t)%z0,vlevel=1,unit="m",&
             direction="-",surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_LAI,                       &
             value=noah33_struc(n)%noah(t)%lai,vlevel=1,unit="-",&
             direction="-",surface_type=LIS_rc%lsm_index)

        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_CH,                        &
             value=noah33_struc(n)%noah(t)%CH,vlevel=1,unit="m s-1",direction="-",&
             surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_CM,                        &
             value=noah33_struc(n)%noah(t)%CM,vlevel=1,unit="m s-1",direction="-",&
             surface_type=LIS_rc%lsm_index)
        call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_MIXRATIO,                  &
             value=noah33_struc(n)%noah(t)%Q1,vlevel=1,unit="kg kg-1",direction="-",&
             surface_type=LIS_rc%lsm_index)
     ...
     if(soilrzmax.gt.0) then 
        IF( 100*soilrz/soilrzmax < noah33_struc(n)%noah(t)%smcwlt ) THEN
           WRSI_TimeStep = 1
        END IF
     endif
     ...
     call LIS_diagnoseSurfaceOutputVar(n,t,LIS_MOC_SurplusWater_TimeStep,     &
          value= noah33_struc(n)%noah(t)%runoff1*LIS_CONST_RHOFW*dt +  &
          noah33_struc(n)%noah(t)%runoff2*LIS_CONST_RHOFW*dt,   &
...
#if (defined COUPLED)
     rho = noah33_struc(n)%noah(t)%psurf/(287.04*noah33_struc(n)%noah(t)%t1)
     q2diag =  noah33_struc(n)%noah(t)%q1 - noah33_struc(n)%noah(t)%qle/&
          (noah33_struc(n)%noah(t)%cqs2* rho)
     t2diag = noah33_struc(n)%noah(t)%t1 - noah33_struc(n)%noah(t)%qh/&
          (rho*noah33_struc(n)%noah(t)%chs2*CP)
...
!            n,t,sncovr,tsoil,NOAH33_STRUC(N)%NOAH(T)%T1
       call LIS_updateTbot(n,t,julian_in,yr,dt,&
            (TSOIL * (1.0 - SNCOVR)) + (NOAH33_STRUC(N)%NOAH(T)%T1 * SNCOVR), &
            tbot)
       noah33_struc(n)%noah(t)%tempbot = tbot
    end if

    noah33_struc(n)%noah(t)%rain_forcing = noah33_struc(n)%noah(t)%rainf
    noah33_struc(n)%noah(t)%snow_forcing = noah33_struc(n)%noah(t)%snowf
!reset 
    noah33_struc(n)%noah(t)%tair = 0 
    noah33_struc(n)%noah(t)%qair = 0 
    noah33_struc(n)%noah(t)%swdown = 0 
    noah33_struc(n)%noah(t)%lwdown = 0
    noah33_struc(n)%noah(t)%uwind = 0
    noah33_struc(n)%noah(t)%vwind = 0 
    noah33_struc(n)%noah(t)%psurf = 0 
    noah33_struc(n)%noah(t)%rainf = 0 
    noah33_struc(n)%noah(t)%rainf_c = 0 
    
    if(noah33_struc(n)%forcing_ch .ne.0) then 
       noah33_struc(n)%noah(t)%ch = 0 
    endif
    ...
    if(LIS_FORC_ALB%selectOpt.eq.1) then 
       noah33_struc(n)%noah(t)%alb=0.0
    endif
    
    if(LIS_FORC_Z0%selectOpt.eq.1) then 
       noah33_struc(n)%noah(t)%z0=0.0
    endif

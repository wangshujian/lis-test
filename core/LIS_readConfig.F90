...
!  call ESMF_ConfigFindLabel(LIS_config,"Model timestep:",rc=rc)
!  do i=1,LIS_rc%nnest
!     call ESMF_ConfigGetAttribute(LIS_config,LIS_rc%nts(i),rc=rc)
!     call LIS_verify(rc,'Model timestep: not defined')
!  enddo

!  LIS_rc%ts = 1000000
!  do i=1,LIS_rc%nnest
!     LIS_rc%ts = min(LIS_rc%ts,LIS_rc%nts(i))
!  enddo
! This is a dummy set. The time manager will be reinitialized to 
...
!------------------------------------------------------------------------
  ...
  write(unit=LIS_logunit,FMT=*)'[INFO] timestep: ',LIS_rc%ts

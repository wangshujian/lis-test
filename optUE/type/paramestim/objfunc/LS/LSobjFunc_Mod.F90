...
!    write(LIS_logunit,*) '[INFO] Here 1 in LSobjFunc_Mod'
...
!    write(LIS_logunit,*) '[INFO] Here 2 in LSobjFunc_Mod: ', ls_ctl%obj_name
    if(ls_ctl%nobjs.gt.1) then
       ...
       write(LIS_logunit,*) 'Reading Least Squares objective function weights ...', &
            LSweightsFile
       ...
       do k=1, ls_ctl%nobjs
          ...
          if(.not.in_namelist) then
             write(LIS_logunit,*) 'The variable ',obj_name_temp, ' is not in list ', &
...
!    write(LIS_logunit,*) '[INFO] Finished LSobjFunc_Mod: '

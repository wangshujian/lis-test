...
! currently limited to one nest
      ...
      if(LIS_rc%nensem(n).le.1) then 
         write(LIS_logunit,*) 'The number of ensembles should be greater than 1'
         write(LIS_logunit,*) 'to run GA. A desired number is = 50'
...
!      enddo
      ...
      do i=1,ga_ctl%nparam
         ...
         do j=1,30
            ...
            if(j.ge.30) then 
               write(LIS_logunit,*) 'The chromosome size is set to 30.'
      ...
      if(ga_ctl%sum_nposs_param.lt.ga_ctl%sum_nbp) then 
         write(LIS_logunit,*) 'Mismatch in sum_nposs_param and sum_nbp'
         write(LIS_logunit,*) 'Program stopping... '
         call LIS_endrun()
      endif
...
!EOP  
      ...
      write(LIS_logunit,*) 'Running GA, generation: ',ga_ctl%genNo
...
!write out in order of ga_ctl parameters list
    ...
    if(LIS_masterproc) then 
       close(40)
       write(LIS_logunit,*) 'GA checkpoint file written ',trim(filen)
    endif
...
!    real, allocatable       :: tempdata(:,:)
    if(ga_ctl%restart.eq."restart") then !restart run
       n = 1    
       write(LIS_logunit,*) 'Reading the GA restart file ..'
       allocate(vardata(LIS_rc%ntiles(n)))
!       allocate(tempdata(ga_ctl%nparam,LIS_rc%nensem(n)))
       ...
       write(LIS_logunit,*) 'Generation Number ',ga_ctl%genNo
...
!                call decode(m,tempdata, ga_struc(t)%iparent) !******temp******kwh
      ...
      close(40)
      write(LIS_logunit,*) 'Finished reading the GA restart file ..'
   endif
